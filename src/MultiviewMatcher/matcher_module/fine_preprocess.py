...
class FinePreprocess(nn.Module):
    ...
    def forward_chunked(self, feature, feature_idx, sample_points, img_idxs, data, scales_relative=None,
                        full_feature_map_patches_num=0):
        ...
        if feature_belonging_mask_sum <= full_feature_map_patches_num:  # sparse feature map
            ...
            features_crop = self._forward_single_scale(features, corresponding_idxs,
                                                       all_sample_points_xy[feature_belonging_mask], data,
                                                       box_scales=scales_relative)  # M * WW * C
    ...
    def forward(self, features, sample_points, img_idxs, data, scales_relative=None):
        ...
        if self.sparse:
            ...
            if self.enable_rescaled_crop:
                ...
                scales_relative = None
            features_crop = self._forward_single_scale(features, points_corresponding_img_idxs, all_sample_points_xy,
                                                       data, box_scales=scales_relative)  # M * WW * C
            # De-format:
            features_crop = rearrange(features_crop, "(b n t) w c -> b n t w c", b=B, n=n_view,
                                      t=n_track)  # B * n_track * n_view * WW * C
        else:
            features_crop = features

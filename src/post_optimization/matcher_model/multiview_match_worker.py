...
def build_model(args, rewindow_size_factor=None, model_idx=None):
    ...
    if rewindow_size_factor is not None:
        ...
        if current_left_win_move_size is not None:
            ...
            matcher_cfg['multiview_matching_test'][
                "left_point_movement_window_size"] = current_left_win_move_size_rescaled
...
def extract_results(
    ...
    return [query_points_refined[mask], query_img_ids[mask], query_pt2D_idxs[mask]], \
        [reference_points_refined[ref_movable_mask], reference_img_ids[ref_movable_mask],
         reference_pt2D_idxs[ref_movable_mask]], time
...
class UpdatedQueryPts:
    ...
    def find_movable_and_update(self, data):
        ...
        query_moveable_mask = []
        query_node = []
        for idx, (query_img_id, query_pt2d_idx) in enumerate(
                zip(data['query_img_ids'][0].numpy(), data['query_pt2d_idxs'][0])):
        ...
        data.update({'query_points': torch.stack(query_node).to(torch.float32)[None],
                     "query_movable_mask": torch.from_numpy(np.stack(query_moveable_mask))[None]})  # 1 * M
...
@torch.no_grad()
def matchWorker(colmap_dataset, matcher, subset_track_idxs=None, dataset_cfgs=None, verbose=True):
    """extract matches from part of the possible image pair permutations"""

    multiview_matching_dataset = MatchingMultiviewData(colmap_dataset, dataset_cfgs,
                                                       worker_split_idxs=subset_track_idxs)
    ...
    for data in tqdm(dataloader, disable=not verbose):
        ...
        [query_points_refined, query_img_ids, query_pt2D_idxs], [ref_points_refined, ref_img_ids,
                                                                 ref_pt2D_idxs], time = extract_results(
